...
contract StakeChefInitializable is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20Metadata;

    // The address of the stake chef factory
    address public immutable STAKE_CHEF_FACTORY;
    ...
    // The block number when CPT mining ends.
    ...
    function initialize(
        ...
        require(msg.sender == STAKE_CHEF_FACTORY, "Not factory");
        require(address(_stakedToken) != address(0), "Invalid stakedToken address");
        ...
        require(_startBlock < _bonusEndBlock, "Start block must be less than bonusEndBlock");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 cptReward = multiplier * rewardPerBlock;
            uint256 adjustedTokenPerShare = accTokenPerShare + (cptReward * PRECISION_FACTOR) / stakedTokenSupply;
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 cptReward = multiplier * rewardPerBlock;
        accTokenPerShare = accTokenPerShare + (cptReward * PRECISION_FACTOR) / stakedTokenSupply;
        lastRewardBlock = block.number;
    }
...
contract StakeChefFactory is Ownable {
    event NewStakeChefContract(address indexed stakeChef);
    ...
    function deployPool(
        ...
        bytes memory bytecode = type(StakeChefInitializable).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address stakeChefAddress;

        assembly {
            stakeChefAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        StakeChefInitializable(stakeChefAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewStakeChefContract(stakeChefAddress);
    }
}